...
def test(epi):
    ...
    acc = correct / count
    writer.add_scalar('test_acc', acc, int(epi))
...
def main():
    ...
    if continue_train == 1:
        # Load specific ckpt
        epi = str(input("episode")).strip()
        print("epi : ", epi)
        latest_file = os.path.join(config['Log']['ckpt_path'], "S-5_Q-27_LR-1e-5_Tmp-1_TopK-10_NRouting-3_NCapsule2_CompDim-4_NLastLayer-2_TAG-induction_2_proposed_fixb_ftune2_dim4_ffa2_Epi-{}.h5".format(epi)) 
...
if __name__ == "__main__":
    ...
    # config.read("config_2_proposed_comp_nomtx_fixb_ftune2_pret_fortest.ini")
    config.read("config_2_proposed_comp_nomtx_fixb_ftune2_dim4_ffa_fortest.ini")
    ...
    main()
    wscalars_to_json("./all_scalars.json")
    writer.close()