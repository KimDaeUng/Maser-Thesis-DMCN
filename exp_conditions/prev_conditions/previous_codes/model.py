...
class DynamicMemoryInduction(nn.Module):
    ...
    def forward(self, m, q):
        ...
        hat_m_ij = squash(self.W(m).reshape(m.shape[0], self.num_caps, self.dim_caps))
        hat_q_ij = squash(self.W(q).reshape(q.shape[0], self.num_caps, self.dim_caps))
        # checking('m', m)
        ...
        tmp_hat_m_ij = hat_m_ij.unsqueeze(0).expand(q.shape[0], m.shape[0], self.num_caps, self.dim_caps).detach()
        # tmp_hat_q_ij = hat_q_ij.unsqueeze(1).repeat(1, self.in_caps, 1, 1).detach()
        # Momory 차원으로 복사(self.in_caps)
        tmp_hat_q_ij = hat_q_ij.unsqueeze(1).expand(q.shape[0], m.shape[0], self.num_caps, self.dim_caps).detach()


        # checking('tmp_hat_m_ij', tmp_hat_m_ij)
        # checking('tmp_hat_q_ij', tmp_hat_q_ij)
        
        a = torch.zeros(q.shape[0], m.shape[0], self.num_caps).to(self.device)
        # checking('a', a)
        ...
        for routing_iter in range(self.num_routing-1):
            d = F.softmax(a.detach()/self.beta, dim=2) # Fix 
            # checking('d', d, True)
            ...
            hat_m_cos_v = torch.sum(torch.mul(
                            ...
                            v.unsqueeze(1).expand(
                                q.shape[0], m.shape[0], self.num_caps, self.dim_caps
                                )), dim=-1)
...
class DynamicTaskMemoryInduction(nn.Module):
    ...
    def forward(self, m, q, episode=None):
        ...
        # self.in_caps = m.shape[0]
        # hat_m_ij = self.W(m).reshape(self.in_caps, self.dim_caps, self.num_caps).permute(0, 2, 1)
        # hat_q_ij = self.W(q).reshape(q.shape[0], self.dim_caps, self.num_caps).permute(0, 2, 1)
        ...
        # Squash Test
        # 배치 차원으로 복사(q.shape[0])
        tmp_hat_m_ij = squash(hat_m_ij.unsqueeze(0)).expand(q.shape[0], self.in_caps, self.num_caps, self.dim_caps).detach()
        # tmp_hat_q_ij = hat_q_ij.unsqueeze(1).repeat(1, self.in_caps, 1, 1).detach()
        # Momory 차원으로 복사(self.in_caps)
        tmp_hat_q_ij = squash(hat_q_ij.unsqueeze(1)).expand(q.shape[0], self.in_caps, self.num_caps, self.dim_caps).detach()
        # checking('tmp_hat_m_ij', tmp_hat_m_ij)
        # checking('tmp_hat_q_ij', tmp_hat_q_ij)
...
class DMRInduction(nn.Module):
    ...
    def forward(self, data, attmask, segid, episode=None):
        ...
        # ret_memory_ : mean vector of support_encoder
        ...
        e_c0 = self.QIM(e_c[0, :, :], query_encoder)
        e_c1 = self.QIM(e_c[1, :, :], query_encoder)
        # print("!!!e_c", e_c.shape)
        # print("!!!e_c1", e_c1.shape)

        if self.is_memory == True:
            self.e_c0 = torch.div(torch.add(self.e_c0, torch.mean(e_c[0, :, :].detach(), dim=0, keepdim=True)), 2).detach()
            self.e_c1 = torch.div(torch.add(self.e_c1, torch.mean(e_c[1, :, :].detach(), dim=0, keepdim=True)), 2).detach()
            # print("self.e_c0.shape after mean: ", self.e_c0.shape)

        else:
            # 수정, 다시 돌려야할지도??
            self.e_c0 = torch.mean(e_c[0, :, :].detach(), dim=0, keepdim=True)
            self.e_c1 = torch.mean(e_c[1, :, :].detach(), dim=0, keepdim=True)
            self.is_memory = True
            # print("self.e_c0.shape : ", self.e_c0.shape)

        # print("chekc : ", self.e_c1.expand(e_c0.shape[1], self.comp_dim).shape)
        # self.e_c_end = torch.cat([self.e_c0.expand(e_c0.shape[0], self.comp_dim), self.e_c1.expand(e_c0.shape[0], self.comp_dim)], dim=0).detach()
        # print("self.e_c_end.shape : ", self.e_c_end.shape)
        
        e_c0_m = self.QIM(self.e_c0, query_encoder)
        e_c1_m = self.QIM(self.e_c1, query_encoder)
        ...
        e_c_end_m = torch.cat([e_c0_m, e_c1_m], dim=0)

        logits = self.simclassifier(e_c_end, query_encoder)
        # print("query_encoder.shape", query_encoder.shape)
        # print("e_c_end.shape", e_c_end.shape)
        logits_2 = self.simclassifier(e_c_end_m, query_encoder)

        logits_t = torch.div(torch.add(logits, logits_2), 2)
        ...
        if (config['Log']['log_DMTR']=='y')&self.training&(episode % self.emb_save_interval == 0):           
            file = {'support_encoder' : support_encoder.detach().cpu().numpy(),
                    ...
                    # 'e_cs' : e_c.detach().cpu().numpy(),
                    # 'e_cq' : e_cq.detach().cpu().numpy(),
                    'e_c_end' : e_c_end.detach().cpu().numpy(),
                    # 'self_e_c_end' : self.e_c_end.detach().cpu().numpy(),
