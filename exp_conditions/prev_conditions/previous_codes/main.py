...
import random
import numpy as np
from model import DMRInduction
from criterion import Criterion, CosineLoss, log_softmax, cross_etrp
...
from utils import plot_grad_flow, accuracy, param_record, hook_fn
...
def train(episode):
    ...
    target = target[n_support:].to(device)
    
    predict = model(data, attmask, segid, episode) # (n_query, n_class)
...
def dev(episode):
    ...
    for data, attmask, segid, target in tqdm.tqdm(dev_loader):
        ...
        predict = model(data, attmask, segid, log_interval+1)
...
def main():
    ...
    for episode in tqdm.trange(1, episodes + 1):
        train(episode)
