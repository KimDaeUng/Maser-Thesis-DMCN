...
class DynamicMemory(nn.Module):
    ...
    def forward(self, encoder_output, memory, iter_routing=3):
        '''
        memory = [C, H]
        '''
        C, K, H = encoder_output.shape
        b = torch.zeros(C, K).to(self.device)
        for _ in range(iter_routing):
            d = F.softmax(b, dim=-1) # [C, K]
            encoder_output_hat = self.l_1(encoder_output) # [C, K, H]
            c_hat = torch.sum(encoder_output_hat*d.unsqueeze(-1), dim=1) # [C, H]
            c = squash(c_hat) # [C, H]
            
            b = b + torch.bmm(encoder_output_hat, c.unsqueeze(-1)).squeeze()
            b = b + torch.bmm(encoder_output_hat, memory.unsqueeze(-1)).squeeze()
            # [C, K, H] @ [C, H, 1] -> [C, K, 1] -> [C, K]
        # write.add_embedding(c, metadata=[0, 1, 2, 3, 4],)
...
class RelationDistMult(nn.Module):
    ...
    def forward(self, class_vector, query_encoder):  # (C,H) (Q,H)
        ...
        query_encoder_r = query_encoder.repeat(self.n_class, 1) # (Q*C, H)
        return torch.mm((class_vector_r * query_encoder_r), self.M).reshape(-1, self.n_class) # [C*Q, H] * [H, 1] -> [C*Q]

class DMRInduction(nn.Module):
    def __init__(self, n_class, k_sample, n_query,
                 n_routing, top_k, comp_dim, relation_out_dim, device):
        ...
        # self.DM_Mem = DynamicMemory(self.comp_dim, device=self.device)
        # self.REL = Relation(H=self.comp_dim, C=n_class, out_size=self.relation_out_dim) 
        # self.REL_TASK = Relation(H=self.comp_dim, C=n_class, out_size=self.relation_out_dim) # noshp_fixm
        ...
        # self.lmda2 = torch.nn.Parameter(torch.tensor(0.5))
    ...
    def forward(self, data, attmask, segid, episode=None):
        ...
        if self.memory_cls_vector is not None:
            memory_n_cur = torch.cat([self.lmda*class_vector, (1-self.lmda)*self.memory_cls_vector], dim=1).reshape(self.n_class, 2, -1) # [C, H]
            ...
            # class_vector_m_clf = self.DM_Mem(e_c, class_vector_m)
            ...
            # score_m = self.REL_TASK(class_vector_m_clf, query_encoder) # noshp_fixm
            # tot_score = torch.sigmoid(self.lmda2*score + (1-self.lmda2)*score_m)
