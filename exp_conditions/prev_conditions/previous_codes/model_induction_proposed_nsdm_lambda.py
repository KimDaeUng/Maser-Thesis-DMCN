...
class DMRInduction(nn.Module):
    def __init__(self, n_class, k_sample, n_query,
        ...
        self.DM2 = DynamicRouting(self.comp_dim, device=self.device)
        ...
        self.lmda = torch.nn.Parameter(torch.tensor(0.5)) # add ndsm + lambda 
    ...
    def forward(self, data, attmask, segid, episode=None):
        ...
        if self.memory_cls_vector is not None:
            ...
            class_vector_m = self.DM2(memory_n_cur) # No share
            ...
            tot_score = torch.sigmoid(self.lmda*score + (1-self.lmda)*score_m) # add ndsm + lambda 
