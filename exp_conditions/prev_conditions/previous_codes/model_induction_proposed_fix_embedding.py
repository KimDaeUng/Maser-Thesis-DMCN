...
class StructuredSelfAttention(nn.Module):
    def __init__(self, emb_dim, lstm_h_dim, att_dim):
        super(StructuredSelfAttention, self).__init__()

        self.bert = BertModel.from_pretrained('bert-base-uncased', output_hidden_states=True)
        self.gru=torch.nn.GRU(emb_dim, lstm_h_dim, batch_first=True, bidirectional=True)
        self.fc1 = torch.nn.Linear(2*lstm_h_dim, att_dim) # att_dim = 64
        self.fc2 = torch.nn.Linear(att_dim, 1) # output = 1
        self.dropout = torch.nn.Dropout(0.1)

    def forward(self, data, attmask, segid):
        embeddings = self.bert(data, attmask, segid)[2]
        hx = embeddings[-2].detach() # [Batch=[support-5 + query-27], Seq_len=512, 768]

        # spt, qry = hx[0:self.n_support], hx[self.n_support:]
        outputs, _ = self.gru(hx) # [Batch=[support-5 + query-27], Seq_len=512, 2*lstm_h_dim]
        # print("outputs 1", outputs.shape)


        x = torch.tanh(self.fc1(outputs)) # [Batch=[support-5 + query-27], Seq_len=512, 64]
        x = self.fc2(x)  # [Batch=[support-5 + query-27], Seq_len=512, 1]
        x = F.softmax(x, dim=1)    # [Batch=[support-5 + query-27], Seq_len=512, 1]
        attention = x.transpose(1, 2) # [Batch=[support-5 + query-27], 1, Seq_len=512]
        sentence_embeddings = attention @ outputs # [Batch=[support-5 + query-27], 1, 2*lstm_h_dim]
        return sentence_embeddings.squeeze(1)
...
class DMRInduction(nn.Module):
    def __init__(self, n_class, k_sample, n_query,
        ...
        self.encoder = StructuredSelfAttention(emb_dim=768, lstm_h_dim=8, att_dim=32)
        ...
        # self.REL_TASK = Relation(H=self.comp_dim, C=n_class, out_size=self.comp_dim) 
    ...
    def forward(self, data, attmask, segid, episode=None):
        ...
        hx  = self.encoder(data, attmask, segid)
        # checking('pooler_output', pooler_output)
        # [N_class*K_samples, in_dim], [N_class*N_querys, in_dim]
        support_encoder, query_encoder = hx[0:self.n_support], hx[self.n_support:]
        ...
        if self.memory_cls_vector is not None:
            ...
            score_m = self.REL(class_vector_m, query_encoder) # fix it to share parameter
