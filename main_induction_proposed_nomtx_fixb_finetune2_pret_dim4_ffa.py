...
import random
import numpy as np
from model_induction_proposed_nomtx_fixb_ftune2nd_pret_ffa import DMRInduction
...
def main():
    ...
    if continue_train == 1:
        # ckpt_path = os.path.join(config['Log']['ckpt_path'], 'S-5_Q-27_LR-1e-5_Tmp-1_TopK-10_NRouting-3_NCapsule2_CompDim-4_NLastLayer-2_TAG-induction_2_proposed_fixb_ftune2_pret_dim4_ffa2_Epi-11600.h5') 
        # list_of_ckpts = glob.glob(ckpt_path)
        # latest_file = max(list_of_ckpts, key=os.path.getctime)
        latest_file = os.path.join(config['Log']['ckpt_path'], 'S-5_Q-27_LR-1e-5_Tmp-1_TopK-10_NRouting-3_NCapsule2_CompDim-4_NLastLayer-2_TAG-induction_2_proposed_fixb_ftune2_pret_dim4_ffa2_Epi-11634.h5') 
        ckpt = torch.load(latest_file)
        model.load_state_dict(ckpt)

        print("Load ckpts : {}".format(latest_file))
        start_epi = int(latest_file.split("Epi-")[-1][:-3])
...
if __name__ == "__main__":
    ...
    config.read("config_2_proposed_comp_nomtx_fixb_ftune2_pret_dim4_ffa.ini")
