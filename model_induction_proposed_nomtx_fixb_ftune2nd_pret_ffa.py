...
class FFA(nn.Module):
    def __init__(self, n_class):
        ...
        self.ffa = nn.Linear(n_class*2, 1)

    def forward(self, score, score_m):  
        scores = torch.cat((score, score_m), dim=1) # (Q, 2C)
        return torch.sigmoid(self.ffa(scores))

# class FFA(nn.Module):
#     def __init__(self, n_class):
#         super(FFA, self).__init__()
#         self.n_class = n_class
#         self.ffa = nn.Linear(n_class*2, n_class*2)
#         self.fc = nn.Linear(n_class*2, n_class)

#     def forward(self, score, score_m):  
#         scores = torch.cat((score, score_m), dim=1) # (Q, 2C)
#         atts =  F.softmax(self.ffa(scores), dim=1)
#         return self.fc(atts*scores)
...
class DMRInduction(nn.Module):
    def __init__(self, n_class, k_sample, n_query,
       ...
       # freeze_layers is a string "1,2,3" representing layer number
        ...
        self.REL = Relation(H=self.comp_dim, C=n_class, out_size=self.comp_dim) 
        self.REL_TASK = Relation(H=self.comp_dim, C=n_class, out_size=self.comp_dim) # noshp_fixm
        self.feedfatt = FFA(self.n_class)
    ...
    def forward(self, data, attmask, segid, episode=None):
        ...
        if self.memory_cls_vector is not None:
            ...
            a_att = self.feedfatt(score, score_m)
            tot_score = torch.sigmoid(a_att*score + (1-a_att)*score_m) # add cross term
        else:
