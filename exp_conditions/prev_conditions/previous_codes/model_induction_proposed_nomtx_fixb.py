...
class DMRInduction(nn.Module):
    def __init__(self, n_class, k_sample, n_query,
        ...
        self.encoder = BertModel.from_pretrained('tmp/finetuend_lm/') # path?
        ...
        self.W_comp = nn.Linear(768, 384)
        self.W_comp2 = nn.Linear(384, 192)
        self.W_comp3 = nn.Linear(192, self.comp_dim)
    ...
    def forward(self, data, attmask, segid, episode=None):
        ...
        hx_ = F.dropout(self.W_comp(hx))
        hx_2 = F.dropout(F.relu(self.W_comp2(hx_)))
        hx_out = F.relu(self.W_comp3(hx_2))
